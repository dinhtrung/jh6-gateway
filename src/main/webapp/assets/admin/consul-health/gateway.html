<ul class="list-unstyled">
  ...
  <% if (components.mail) { %>
  <li><strong>Mail Server:</strong> <span class="float-right badge badge-${ (components.mail.status === 'UP' ? 'success' : 'danger')}">${ components.mail.status }</span>
    <ul>
      <li><strong>Location:</strong> <code class="float-right">${ components.mail.details.location }</code></li>
    </ul>
  </li>
  <% } %>
